...
from MyDeepAgent import RLCardAgent

#xjhm
EnvCard2RealCard = {19: '3', 4: '4', 5: '5', 6: '6', 7: '7',
                    8: '8', 9: '9', 10: 'T', 11: 'J', 12: 'Q',
                    13: 'K', 15: 'A', 17: '2',  20: 'X', 30: 'D'}

#xjhm
RealCard2EnvCard = {'3': 19, '4': 4, '5': 5, '6': 6, '7': 7,
                    '8': 8, '9': 9, 'T': 10, 'J': 11, 'Q': 12,
                    'K': 13, 'A': 15, '2': 17, 'X': 20, 'D': 30}

pretrained_dir = 'pretrained/waken_pretrained'
players = []
for position in ['landlord', 'landlord_down', 'landlord_up']:
    players.append(RLCardAgent(position))
    #players.append(RLCardAgent(position, pretrained_dir, use_onnx=True))
...
def predict():
    if request.method == 'POST':
        try:
            ...
            # xjhm
            if len(three_landlord_cards) < 0 or len(three_landlord_cards) > 4:
                return jsonify({'status': 6, 'message': 'the number of landlord cards should be 0-4'})
            ...
            #print(info_set.legal_actions)
            # Prediction
            #legal_actions = _get_legal_card_play_actions(player_hawinnd_cards, rival_move)
...
def predict():
    if request.method == 'POST':
        try:
            ...
            actions = _get_legal_card_play_actions(player_hand_cards, rival_move)
            actions = ','.join([''.join([EnvCard2RealCard[a] for a in action]) for action in actions])
            actions_confidence = np.random.dirichlet(np.ones(len(actions)), size = 1).tolist()[0]
            result = {}
            win_rates = {}
...
def legal():
    if request.method == 'POST':
        try:
            player_hand_cards = [RealCard2EnvCard[c] for c in request.form.get('player_hand_cards')]
            #print(request.form.get('rival_move'))
...
def _get_legal_card_play_actions(player_hand_cards, rival_move):
    ...
    elif rival_move_type == md.TYPE_11_SERIAL_BOMB:
        all_moves = mg.gen_type_11_serial_bomb(repeat_num=rival_move_len)
        moves = ms.filter_type_11_serial_bomb(all_moves, rival_move)
