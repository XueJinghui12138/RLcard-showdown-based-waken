from .utils import MIN_SINGLE_CARDS, MIN_PAIRS, MIN_TRIPLES,MIN_BOMB, select
import collections
import itertools
...
class MovesGener(object):
    ...
    def gen_type_11_serial_bomb(self, repeat_num=0):
        single_bomb = list()
        for k, v in self.cards_dict.items():
            if v >= 4:
                single_bomb.append(k)

        return self._gen_serial_moves(single_bomb, MIN_BOMB, repeat=4, repeat_num=repeat_num)
    ...
    def gen_moves(self):
        ...
        moves.extend(self.gen_type_11_serial_bomb())
