...
def is_continuous_seq_dict(move):
    i = 0
    while i < len(move) - 1:
        if move[i + 1][0] - move[i][0] != 1 or move[i + 1][1] != move[i][1]:
            return False
        i += 1
    return True
...
def get_move_type(move):
    ...
    move_dict = sorted(move_dict.items(), key=lambda x: x[0], reverse=False)

    if is_continuous_seq_dict(move_dict):
        if move_dict[0][1] == 1:
            return {'type': TYPE_8_SERIAL_SINGLE, 'rank': move[0], 'len': len(move)}
        elif move_dict[0][1] == 2:
            return {'type': TYPE_9_SERIAL_PAIR, 'rank': move[0], 'len': len(move)}
        elif move_dict[0][1] == 3:
            return {'type': TYPE_10_SERIAL_TRIPLE, 'rank': move[0], 'len': len(move)}
        elif move_dict[0][1] == 4:
            return {'type': TYPE_11_SERIAL_BOMB, 'rank': move[0], 'len': len(move)}
