...
EnvCard2RealCard = {19: '3', 4: '4', 5: '5', 6: '6', 7: '7',
                    ...
                    13: 'K', 15: 'A', 17: '2', 20: 'B', 30: 'R'}
RealCard2EnvCard = {'3': 19, '4': 4, '5': 5, '6': 6, '7': 7,
                    '8': 8, '9': 9, 'T': 10, 'J': 11, 'Q': 12,
                    'K': 13, 'A': 15, '2': 17, 'B': 20, 'R': 30}
...
class RLCardAgent(object):
    ...
    def __init__(self, position):
        ...
        self.CARD_TYPE = None
        card_type_path = 'card_type.json'
        with open(card_type_path, 'r') as f:
            data = json.load(f, object_pairs_hook=OrderedDict)
        self.CARD_TYPE = (data, list(data), set(data))
    ...
    def act(self, infoset):
        try:
            ...
            last_move = infoset.last_moves.copy()
            ...
            # the rule of following cards
                ...
                the_type = self.CARD_TYPE[0][last_move][0][0]
                chosen_action = ''
                rank = 1000
                for ac in infoset.legal_actions:
                    ...
                    _ac = ''.join(_ac)
                    if _ac != '' and the_type == self.CARD_TYPE[0][_ac][0][0]:
                        if int(self.CARD_TYPE[0][_ac][0][1]) < rank:
                            rank = int(self.CARD_TYPE[0][_ac][0][1])
                            chosen_action = _ac
                if chosen_action != '':
                    action = [char for char in chosen_action]
                    for i, c in enumerate(action):
            ...
            if len(chain) >= 3:
                ...
                if min_count != 0:
                    ...
                    for num in range(len(chain)):
                        str_chain += str_card[start + num+1]
...
def combine_cards(hand):
    ...
    '''
    comb = {'bomb_chain':[],'bomb': [], 'trio': [], 'trio_chain': [],
    ...
    chains, hand_list = pick_chain(hand_list, 3)
    comb['trio_chain'] = chains

    chains, hand_list = pick_chain(hand_list, 4)
    comb['bomb_chain'] = chains
