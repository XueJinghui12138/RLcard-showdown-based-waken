...
//xjhm
let threeLandlordCards = shuffleArray(sortDoudizhuCards(shuffledDoudizhuDeck.slice(0, 4)));
...
//xjhm
let initHands = [
    shuffledDoudizhuDeck.slice(4, 20),
    shuffledDoudizhuDeck.slice(20, 36),
    shuffledDoudizhuDeck.slice(36, 52),
];
...
function PvEDoudizhuDemoView() {
    ...
    const requestApiPlay = async () => {
        ...
        try {
            ...
            if (data.status !== 0) {
                if (data.status === -1) {
                    ...
                    if (apiRes.data.legal_action === '') {
                        ...
                        Message({
                            message: '1Error receiving prediction result, please try refresh the page',
                            type: 'error',
                            showClose: true,
                        });
                    }
            ...
            Message({
                message: '2Error receiving prediction result, please try refresh the page',
                type: 'error',
                showClose: true,
            });
        }
    ...
    const handleCloseGameEndDialog = () => {
        ...
        //xjhm
        threeLandlordCards = shuffleArray(sortDoudizhuCards(shuffledDoudizhuDeck.slice(0, 4)));
